...
function player(slot,index){return `${DEFAULT_PLAYERS[hash(`${slot}:p:${index}`)%DEFAULT_PLAYERS.length]}(${ALIASES[hash(`${slot}:a:${index}`)%ALIASES.length]})`;}
...
export function createVirtualSportEngine({pool,deterministicUuid,config}){
  ...
  function pair(slot,index){
    ...
    return {home:`${p[0]} (${player(slot,index*2)})`,away:`${p[1]} (${player(slot,index*2+1)})`};
  }
  ...
  function markets(event){
    ...
    const odds=winnerOdds(h,a);
    const total=(h+a)+0.5;
    odds.push({code:'O',label:`Más de ${Math.max(1,total)}`,odds:Number((1.82+(hash(event.id)%18)/100).toFixed(2))});
    odds.push({code:'U',label:`Menos de ${Math.max(1,total)}`,odds:Number((1.82+((hash(event.id)>>3)%18)/100).toFixed(2))});
    return odds;
  }
  async function ensureMarkets(event){
    const key=`${cfg.source}:${event.id}:main`;
    ...
    await pool.query(`INSERT INTO markets(id,event_id,name,market_type,status,external_key,pricing_source,pricing_updated_at)
      ...
      ON CONFLICT(external_key) WHERE external_key IS NOT NULL DO UPDATE SET status='OPEN',pricing_source=EXCLUDED.pricing_source,pricing_updated_at=NOW()`,
      [mid,event.id,`Mercados ${cfg.sport} 2H2`,cfg.marketType,key,cfg.source]);
    const seen=[];
    for(const o of markets(event)){
