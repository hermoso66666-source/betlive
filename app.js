...
let viewRequestId=0,eventsLoading=false;
...
async function loadEvents(forceLive=true){
  ...
  try{
    ...
    if(requestId!==viewRequestId || sport!==requestedSport) return;
    ...
    toast(e.message);
  }finally{if(requestId===viewRequestId)eventsLoading=false;}
}
async function loadUpcoming(){const requestId=++viewRequestId;eventsLoading=true;try{hotMode=false;raceMode=false;feedMode="upcoming";liveOnly=false;$("#matches").innerHTML="<div class=empty>Cargando partidos reales...</div>";const d=await api("/api/events/upcoming-real");if(requestId!==viewRequestId)return;M=normalizeEvents(d.events);$("#viewTitle").textContent="📅 PRÓXIMOS PARTIDOS";const src=d.source||"BETLIVE";$("#feedNotice").textContent=d.error?`BetLive conserva los partidos disponibles. Fuente adicional no disponible: ${d.error}`:`Fuente: ${src}. Los mercados internos de BetLive no dependen de las cuotas externas.`;$("#feedNotice").classList.remove("hidden");render()}catch(e){if(requestId!==viewRequestId)return;$("#matches").innerHTML=`<div class=empty>No se pudieron cargar los próximos partidos.<br><small>${escapeHtml(e.message)}</small><br><button id="retryUpcoming" class="red" style="margin-top:12px">Reintentar</button></div>`;const b=$("#retryUpcoming");if(b)b.onclick=loadUpcoming;toast(e.message)}finally{if(requestId===viewRequestId)eventsLoading=false;}}
...
function render(){
  ...
  $("#matches").innerHTML=a.map(m=>{
    ...
    const marketHtml=visibleMarkets.map(mk=>`<div class="live-market"><b>${escapeHtml(mk.name)}</b><div class="live-market-options">${(mk.selections||[]).slice(0,6).map(s=>{
      const selected=slip.some(x=>x.key===`${m.id}:${s.id}`);
      const marketCode = s.code === 'L' ? 'L' : s.code === 'E' ? 'E' : s.code === 'V' ? 'V' : '';
      return `<button type="button" class="odd ${selected?"sel":""}" aria-pressed="${selected}" data-id="${m.id}" data-sid="${s.id}" ${s.status!=="OPEN"||m.status==="CLOSED"?"disabled":""}><small>${marketCode?`<b class="lev-code">${marketCode}</b> `:''}${escapeHtml(s.label)}</small><b>${displayOdds(s.odds)}</b></button>`;
    }).join("")}</div></div>`).join("");
...
function pickSelection(eventId,selectionId){
  ...
  const key=`${eventId}:${selectionId}`;const x={id:eventId,key,selectionId,odd:Number(s.odds),home:m.home,away:m.away,league:m.league,label:`${market?.name||"Mercado"}: ${s.label}`,code:s.code};
  ...
  const p=slip.findIndex(y=>y.key===key || (!y.key && y.selectionId===selectionId && y.id===eventId));
  if(p>=0)slip.splice(p,1);else{if(type==="single")slip=[];slip.push(x)}
  render()
}
function pick(id,i){let m=M.find(x=>x.id===id),o=m?.odds[i];if(!o)return;let x={id,key:`${id}:${o.id}`,i,selectionId:o.id,odd:o.odd,home:m.home,away:m.away,league:m.league,label:o.label,code:o.code};if(quickMode){quick=x;$("#qselection").innerHTML=`<b>${x.home} vs ${x.away}</b><br>${x.league} · ${x.label} · ${americanOdds(x.odd)} (${x.odd.toFixed(2)})`;$("#qbet").disabled=false;return}let p=slip.findIndex(y=>y.key===x.key || (!y.key && y.id===id && y.selectionId===o.id));if(p>=0)slip.splice(p,1);else{if(type==="single")slip=[];slip.push(x)}render()}
...
$("#showBets").onclick=()=>{$("#events").classList.add("hidden");$("#bets").classList.remove("hidden");loadTickets("all")};
let betsSyncTimer=null;
function startBetsSync(){
  clearInterval(betsSyncTimer);
  betsSyncTimer=setInterval(()=>{
    if(document.visibilityState==="visible"&&!$("#bets").classList.contains("hidden")&&user)loadTickets("all");
  },5000);
}
startBetsSync();
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"&&!$("#bets").classList.contains("hidden")&&user)loadTickets("all")});$("#showLive").onclick=async()=>{$("#bets").classList.add("hidden");$("#events").classList.remove("hidden");await loadEvents(true)};
