const HOT_CATEGORY_LABEL = "HOT · Liga Virtual";
...
const HOT_TEAM_POOLS={
  ...
  "Básquetbol":[["Los Angeles Stars","Boston Greens"],["Golden Bay","Chicago Bullsmen"],["Miami Waves","New York Knights"],["Dallas Rockets","Phoenix Sunside"]],
  "Béisbol":[["Los Angeles Bats","New York Kings"],["Boston Redcaps","Chicago Cubsmen"],["Texas Rangers FC","Miami Marlinside"],["San Diego Padres Club","Atlanta Bravesmen"]],
  "Tenis":[["Madrid Open","London Aces"],["Paris Smash","New York Serve"],["Roma Masters","Tokyo Rackets"],["Miami Open","Toronto Nets"]]
};
...
function hotPlayer(slot,index){
  ...
  const alias=HOT_PLAYER_ALIASES[(hotHash(slot*53+index*11))%HOT_PLAYER_ALIASES.length];
  return `${HOT_PLAYER_NAMES[i]}(${alias})`;
}
function hotIsAllowedStart(date,sport){
  if(sport==="Fútbol") return true;
  const h=Number(new Intl.DateTimeFormat("en-US",{timeZone:"America/Mexico_City",hour:"2-digit",hour12:false}).format(new Date(date)));
  return h>=HOT_OTHER_START_HOUR && h<HOT_OTHER_END_HOUR;
}
function hotTeamName(sport,slot,index){
  const pool=HOT_TEAM_POOLS[sport]||HOT_TEAM_POOLS["Fútbol"];
  const pair=pool[(slot+index)%pool.length];
  return `${pair[0]} (${hotPlayer(slot,index*2)})`;
}
function hotMakePair(sport,slot,index){
  const pool=HOT_TEAM_POOLS[sport]||HOT_TEAM_POOLS["Fútbol"];
  const pair=pool[(slot+index)%pool.length];
  return {
    home:`${pair[0]} (${hotPlayer(slot,index*2)})`,
    away:`${pair[1]} (${hotPlayer(slot,index*2+1)})`
  };
}
function hotStatsFor(event){
  const elapsed=Math.max(0,Math.min(HOT_DURATION_MINUTES,Number(event.live_elapsed)||0));
  const seed=hotHash(String(event.id).split("-").reduce((a,x)=>a+x.charCodeAt(0),0));
  const home=Number(event.home_score)||0, away=Number(event.away_score)||0;
  const base=(seed%41)+30;
  return {
    elapsed,
    possession:[Math.max(35,Math.min(65,base+(home-away)*2)),0],
    shots:[Math.max(home+1,(seed%7)+elapsed),Math.max(away+1,(seed%6)+Math.floor(elapsed*.8))],
    shotsOnTarget:[Math.max(home,(seed%4)+Math.floor(elapsed/3)),Math.max(away,(seed%3)+Math.floor(elapsed/4))],
    attacks:[(seed%18)+elapsed*3,(seed%16)+elapsed*3],
    corners:[Math.min(12,Math.floor(elapsed/3)+(seed%3)),Math.min(12,Math.floor(elapsed/4)+((seed>>2)%3))],
    fouls:[(seed%5)+Math.floor(elapsed/2),((seed>>3)%5)+Math.floor(elapsed/2)],
    winPct:[0,0,0]
  };
}
function hotWinPct(home,away,elapsed){
  const t=Math.max(0,Math.min(1,elapsed/HOT_DURATION_MINUTES)), d=home-away;
  let hp=.40+d*.13+t*.05, ap=.40-d*.13+t*.05, dp=1-hp-ap;
  if(d>0){hp+=.08*t;ap-=.05*t;dp-=.03*t}
  if(d<0){ap+=.08*t;hp-=.05*t;dp-=.03*t}
  const vals=[hp,dp,ap].map(x=>Math.max(.03,x)); const sum=vals.reduce((a,b)=>a+b,0);
  return vals.map(x=>Math.round(x/sum*100));
}
function hotMarketOdds(event){
  const elapsed=Math.max(0,Math.min(HOT_DURATION_MINUTES,Number(event.live_elapsed)||0));
  const h=Number(event.home_score)||0,a=Number(event.away_score)||0;
  const p=hotWinPct(h,a,elapsed);
  const margin=.07;
  const probs=p.map(x=>Math.max(.02,x/100));
  const prices=probs.map(x=>Math.max(1.01,1/(x*(1+margin))));
  const american=o=>o>=2?Math.round((o-1)*100):Math.round(-100/(o-1));
  return [
    {code:"L",label:event.home_team,odds:Number(prices[0].toFixed(2)),american:american(prices[0])},
    {code:"E",label:"Empate",odds:Number(prices[1].toFixed(2)),american:american(prices[1])},
    {code:"V",label:event.away_team,odds:Number(prices[2].toFixed(2)),american:american(prices[2])}
  ];
}
...
app.get("/api/events",async(req,res)=>{
  try{
    ...
    const {rows}=await pool.query(`
      ...
      WHERE e.status IN ('OPEN','LIVE') AND COALESCE(e.hot_enabled,FALSE)=FALSE
...
async function queryUpcomingEvents(){
  const {rows}=await pool.query(`
    ...
    FROM sports_events e LEFT JOIN markets m ON m.event_id=e.id
    WHERE e.status='OPEN' AND e.starts_at>=NOW() AND COALESCE(e.hot_enabled,FALSE)=FALSE
...
app.post("/api/admin/hot/generate",auth,requireAdmin,async(req,res)=>{try{const r=await seedHotSchedule();await advanceHotEvents();await audit(req.user.id,"HOT_GENERATE","hot",null,r);res.json({ok:true,...r})}catch(e){res.status(500).json({ok:false,error:e.message})}});
app.post("/api/admin/hot/event/:id/control",auth,requireAdmin,async(req,res)=>{
  try{
    ...
    if(status!==undefined){fields.push(`status=$${vals.length+1}`);vals.push(String(status).toUpperCase())}
    ...
    if(winner!==undefined){fields.push(`hot_winner=$${vals.length+1}`);vals.push(String(winner).slice(0,10))}
    ...
    await ensureHotMarket(q.rows[0]);await audit(req.user.id,"HOT_CONTROL","event",req.params.id,{status,homeScore,awayScore,elapsed,winner,locked});
...
async function generateInternalMarketsForAllActiveEvents(){
  ...
  const {rows}=await pool.query(`
    ...
    FROM sports_events
    WHERE COALESCE(hot_enabled,FALSE)=FALSE AND (status='LIVE'
...
async function ensureHotMarket(event){
  const key=`hot:2h2:${event.id}:winner`;
  ...
  await pool.query(`INSERT INTO markets(id,event_id,name,market_type,status,external_key,pricing_source,pricing_updated_at)
    VALUES($1,$2,'Ganador 2H2','HOT_2H2','OPEN',$3,'HOT_ENGINE',NOW())
    ON CONFLICT(external_key) WHERE external_key IS NOT NULL DO UPDATE SET status='OPEN',pricing_source='HOT_ENGINE',pricing_updated_at=NOW()`,
    [mid,event.id,key]);
  const sels=hotMarketOdds(event);
  const seen=[];
  for(const x of sels){
    const sk=`${key}:${x.code}`; seen.push(sk); const sid=deterministicUuid(sk);
    await pool.query(`INSERT INTO market_selections(id,market_id,label,code,odds,status,external_key)
      VALUES($1,$2,$3,$4,$5,'OPEN',$6)
      ON CONFLICT(external_key) WHERE external_key IS NOT NULL DO UPDATE SET label=EXCLUDED.label,odds=EXCLUDED.odds,status='OPEN'`,
      [sid,mid,x.label,x.code,x.odds,sk]);
  }
...
async function createHotEvent(startsAt,sport,slot,index){
  const pair=hotMakePair(sport,slot,index);
...
async function seedHotSchedule(){
  if(!HOT_ENABLED) return {created:0};
  const now=Date.now();
  let created=0;
  for(const sport of HOT_SPORTS){
    let idx=0;
    const start=Math.ceil(now/HOT_INTERVAL_MS)*HOT_INTERVAL_MS;
    const end=now+HOT_ROLLING_HOURS*60*60*1000;
    for(let t=start;t<=end;t+=HOT_INTERVAL_MS){
      if(!hotIsAllowedStart(t,sport)) continue;
      const slot=hotRotationSlot(t);
      const key=`hot:${sport}:${t}`;
      const exists=await pool.query("SELECT 1 FROM sports_events WHERE external_source='HOT_ENGINE' AND external_id=$1 LIMIT 1",[key]);
      if(!exists.rows[0]){await createHotEvent(new Date(t).toISOString(),sport,slot,idx++);created++;}
    }
  }
  return {created};
}
async function advanceHotEvents(){
  if(!HOT_ENABLED) return {updated:0,closed:0};
  const q=await pool.query(`SELECT * FROM sports_events WHERE hot_enabled=TRUE AND status IN ('OPEN','LIVE') AND starts_at<=NOW()+INTERVAL '1 minute' ORDER BY starts_at LIMIT 1000`);
  let updated=0,closed=0;
  for(const e of q.rows){
    if(e.hot_locked) continue;
    const elapsed=Math.floor((Date.now()-new Date(e.starts_at).getTime())/60000);
    ...
    if(sport==="Fútbol" && minute>0){
      home=Math.min(4,Math.floor(((seed%7)+minute*2)/13));
      away=Math.min(4,Math.floor((((seed>>4)%7)+minute*2)/15));
    }else if(sport==="Básquetbol"){
      home=Math.min(99,Math.floor(8+minute*2.4+(seed%5)));
      away=Math.min(99,Math.floor(7+minute*2.2+((seed>>3)%5)));
    }else if(sport==="Béisbol"){
      home=Math.min(8,Math.floor(((seed%8)+minute)/6));
      away=Math.min(8,Math.floor((((seed>>5)%8)+minute)/7));
    }else{
      home=Math.min(3,Math.floor(((seed%5)+minute)/4));
      away=Math.min(3,Math.floor((((seed>>5)%5)+minute)/5));
    }
    const stats=hotStatsFor({...e,live_elapsed:minute,home_score:home,away_score:away});
    stats.winPct=hotWinPct(home,away,minute);
    stats.possession[1]=100-stats.possession[0];
    const status=minute>=HOT_DURATION_MINUTES?"CLOSED":"LIVE";
    const liveStatus=minute>=HOT_DURATION_MINUTES?"Finalizado HOT":`En vivo HOT · ${minute}'`;
    await pool.query(`UPDATE sports_events SET status=$1,home_score=$2,away_score=$3,live_elapsed=$4,live_status=$5,score_source='HOT_ENGINE',score_confidence=100,score_updated_at=NOW(),hot_stats=$6 WHERE id=$7`,
      [status,home,away,minute,liveStatus,JSON.stringify(stats),e.id]);
    const latest={...e,home_score:home,away_score:away,live_elapsed:minute,hot_stats:stats,status};
    await ensureHotMarket(latest);
    updated++;
...
async function startHotEngine(){
  ...
  setInterval(async()=>{try{await advanceHotEvents()}catch(e){console.error("HOT cycle:",e.message)}},60*1000);
}
