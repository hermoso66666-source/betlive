...
export function generateLEVMarket({historical={},live={},betting={},config={}}={}){
  ...
  const edge=clamp((hs-as)*.30+(hf-af)*.20,-.25,.25);
  ...
  const base=normalize3(.45+edge,.28-Math.abs(edge)*.10,.27-edge);
  const liveP=minute>0?normalize3(.40+scoreEdge+pressureEdge,.30-Math.abs(scoreEdge)*.20,.30-scoreEdge-pressureEdge):base;
  const ba=Math.max(0,safeNum(betting.homeAmount)),bd=Math.max(0,safeNum(betting.drawAmount)),bc=Math.max(0,safeNum(betting.awayAmount));
  const betP=normalize3(ba||1,bd||1,bc||1);
  const totalW=cfg.historyWeight+cfg.formWeight+cfg.liveWeight+cfg.bettingWeight;
  ...
  return {selections:[{code:'L',probability:+priced[0].toFixed(6),odds:+odds[0].toFixed(2)},{code:'E',probability:+priced[1].toFixed(6),odds:+odds[1].toFixed(2)},{code:'V',probability:+priced[2].toFixed(6),odds:+odds[2].toFixed(2)}]};
}
